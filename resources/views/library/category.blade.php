<x-dashboard-layout :title="$category->name">
    <section class="py-3">
        <h3 class="bg-milk-200 py-3 px-2 pl-4 font-bold text-md text-gray-900 underline">Category - {{ strtoupper($category->name) }} ( {{ $category->books()->count() }} )</h3>
        <section class="grid grid-cols-6 gap-x-3 gap-y-4 px-3 py-4">
            @forelse($category->books()->latest()->get() as $book)
                <div class="">
            ...
            @empty
                <div class="col-span-6 text-center font-bold text-lg py-6 text-gray-700 uppercase">
                    <p>No Books In Shelf</p>
                </div>
            @endforelse
        </section>
    </section>
