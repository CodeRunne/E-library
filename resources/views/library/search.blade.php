<x-dashboard-layout title="Search">
    <section class="py-3">
        <h3 class="bg-milk-200 py-3 px-2 pl-4 font-bold text-lg text-gray-700">{{ $results->count() < 1 ? 'No' : $results->count() }} Results Found For ' {{ $query }} '</h3>
        <section class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-x-3 gap-y-4 px-3 py-4"> 
